...
function App() {
  ...
  // Click outside & Escape key listener for search dropdown
  useEffect(() => {
  ...
  // Fetch weather data
  const fetchWeather = async (city, saveToHistoryFlag = false, lat = null, lon = null) => {
    ...
    try {
      ...
      if (lat && lon) {
        data = await getWeatherDataByCoords(lat, lon, units);
      } else {
        const rawName = city === 'Hyderabad (India)' ? 'Hyderabad, India' : city;
        const cleanCityName = (rawName || 'Vehari')
          .replace(/\s+District$/i, '')
          .replace(/\s+Division$/i, '')
          .replace(/\s+Tehsil$/i, '')
          .replace(/^Current Location$/i, 'Vehari')
          .trim();
        data = await getWeatherData(cleanCityName || 'Vehari', units);
      }
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${getThemeGradient()} ${theme === 'dark' ? 'text-gray-100' : 'text-slate-800'} transition-all duration-1000 flex flex-col font-sans relative overflow-x-clip pb-12`}>
      ...
      {/* Fixed Landing Navbar / Header */}
      <header className="fixed top-0 left-0 right-0 z-[9999] overflow-visible bg-white/80 dark:bg-slate-950/85 backdrop-blur-xl border-b border-slate-200 dark:border-white/10 shadow-lg transition-all duration-300">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex flex-col md:flex-row gap-3 md:gap-4 items-center justify-between w-full max-w-full">
            ...
            <div className="flex flex-col md:flex-row items-center gap-2 sm:gap-3 w-full md:w-auto md:flex-grow md:max-w-2xl md:justify-end z-30 relative">
              <div ref={searchContainerRef} className="relative z-[10000] w-full md:flex-grow">
                <form onSubmit={handleSearchSubmit} className="relative w-full">
                  <input
                    ...
                    className="w-full glass-input text-xs sm:text-sm text-slate-800 dark:text-white pl-9 sm:pl-10 pr-9 sm:pr-10 py-2 sm:py-2.5 rounded-full outline-none transition-all duration-300 font-outfit"
                  />
                ...
                <AnimatePresence>
                  {showSearchSuggestions && (
                    <motion.div
                      ...
                      className="absolute top-full left-0 mt-2 w-full rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden z-[10001] animate-in fade-in slide-in-from-top-2 duration-200"
                    >
                      {!cityInput.trim() ? (
                        <>
                          {/* Recent Searches */}
                          {searchHistory.length > 0 && (
                            <>
                              <div className="px-4 py-2 text-xs font-bold text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] tracking-wider uppercase flex justify-between items-center">
                                ...
                                <button
                                  ...
                                  className="text-[10px] text-blue-600 dark:text-blue-400 hover:text-blue-500 font-semibold transition-all duration-150 uppercase"
                                >
                                  {t('navbar.clearAll')}
                                </button>
                              </div>
                              <div className="flex flex-col gap-0.5 p-1.5 border-b border-slate-100 dark:border-white/5 bg-white dark:bg-slate-900">
                                {searchHistory.map((city) => (
                                  <div
                                    key={city}
                                    className="group flex items-center justify-between px-3 py-2 text-xs rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/80 transition-colors duration-150 cursor-pointer"
                                  >
                                    <button
                                      type="button"
                                      onMouseDown={() => {
                                      ...
                                      className="text-left text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white font-medium flex-grow truncate flex items-center gap-2.5"
                                    >
                                    ...
                                    <button
                                      ...
                                      className="text-slate-400 hover:text-red-500 p-1 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                          ...
                          <div className="px-4 py-2 text-xs font-bold text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] tracking-wider uppercase flex items-center gap-1.5">
                            <span>⭐</span>
                            <span>{t('navbar.popularLocations')}</span>
                          </div>
                          <div className="grid grid-cols-2 gap-1 p-2 bg-white dark:bg-slate-900/90 backdrop-blur-md">
                            {popularCities.map((city) => (
                              <button
                                ...
                                className="text-left px-3 py-2 text-xs text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl transition-all duration-150 flex items-center gap-1.5"
                              >
                                <span className="text-slate-400 dark:text-slate-500 text-xs">📍</span>
                                <span className="truncate">{city}</span>
                              </button>
      ...
      <main className="pt-36 sm:pt-28 md:pt-24 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex-grow relative z-0">
